...
- [#2174](https://github.com/openmls/openmls/pull/2173): Simplified the HPKE construction for targeted messages to match the updated draft (`targeted-messages-draft` feature). The recipient and sender leaf indices are now bound through the HPKE info and the `authenticated_data` field is used directly as the HPKE AAD. The `TargetedMessageTBM` struct is removed and the sender uses a single-shot PSK-mode seal. The `OpenMlsCrypto` trait method `hpke_seal_psk_resolved_aad` is replaced by `hpke_seal_psk`.
