...
- `CommitBuilder::build_with_new_signer` now always generates an UpdatePath, so the new signature key is installed in the committer's leaf even when no proposal requires a path. Previously, a new signer passed to an otherwise pathless commit was silently ignored. It also rejects a new signer whose signature scheme does not match the group's ciphersuite with `CreateCommitError::InvalidSignerCiphersuite`.
