...
- [#XXXX](https://github.com/openmls/openmls/pull/XXXX): `MlsGroup::create_message` (the non-`virtual-clients-draft` build) now returns `CreateMessageError<Provider::StorageError>` instead of the non-generic `CreateMessageError`, so it can report the specific storage error above. `CreateMessageError` gained a `StorageError(StorageError)` variant.

### Fixed

- [#XXXX](https://github.com/openmls/openmls/pull/XXXX): A storage-provider failure while framing an outgoing message (application message via `MlsGroup::create_message`, or any proposal/commit/external-commit/self-removal handshake message) previously surfaced as a misleading `LibraryError::custom("Malformed plaintext")` instead of the real cause. It now surfaces via the affected error type's existing storage-error variant (e.g. `CommitBuilderStageError::KeyStoreError`, `ProposeAddMemberError::StorageError`).
