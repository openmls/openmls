...
macro_rules! storage_helpers {
    ($version:expr, $err:ty, $ser:path, $de:path) => {
        ...
        /// `epoch_id` if neither retained key package material nor the
        /// emulation bindings of a group still reference it.
        #[cfg(feature = "virtual-clients-draft")]
        fn delete_vc_state_if_unreferenced<EpochId: Key<$version>>(
            ...
            if epoch_has_emulation_binding(epoch_id, bound_epoch_lists)? {
                return Ok(false);
            }
