...
# tests do not pass with them. Enabling `openmls/virtual-clients-draft` gates
# `MlsGroup::self_update` and `add_members` behind `test-utils`, which these tests
# do not enable, so the operability tests cannot exercise them. As a result the
# vc-draft storage-provider code, including `vc_derivation_epoch_log`, is not
# covered here. Re-enable these lines once the tests no longer need those gated
# APIs.
