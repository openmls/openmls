...
use openmls::{group::GroupEpoch, messages::Proposal, prelude::*, schedule::MembershipKey};
...
fn build_handshake_messages(
    ...
    group.set_epoch(epoch);
    let membership_key = MembershipKey::from_secret(Secret::random_test(
        group.ciphersuite(),
        None, /* MLS version */
    ));
    ...
    let ciphertext = MlsCiphertext::try_from_plaintext_test(&plaintext, group, leaf, 0)
...
fn build_application_messages(
    ...
    .unwrap();
    plaintext.remove_membership_tag();
    let ciphertext = match MlsCiphertext::try_from_plaintext_test(&plaintext, group, leaf, 0) {
        Ok(c) => c,
