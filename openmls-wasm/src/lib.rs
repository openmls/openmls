...
impl Group {
    ...
    pub fn store_pending_proposal(
        ...
        todo!("Need to find a way to convert the &[u8] to a Proposal");
        // Ok(self
        //     .mls_group
    ...
    pub fn process_message(
        ...
        match msg.into_content() {
            ...
            | openmls::framing::ProcessedMessageContent::ExternalJoinProposalMessage(proposal) => {
                Ok(proposal.tls_serialize_detached()?)
            }
...
mod tests {
    ...
    fn basic() {
        ...
        let queued_proposal = chess_club_bob
            .process_message(&mut bob_provider, &add_msg.proposal)
            .map_err(js_error_to_string)
            .unwrap();
        chess_club_bob
            .store_pending_proposal(&bob_provider, &queued_proposal)
            .map_err(js_error_to_string)
            .unwrap();
        chess_club_bob
