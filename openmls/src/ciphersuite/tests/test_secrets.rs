...
fn secret_init(ciphersuite: Ciphersuite, backend: &impl OpenMlsCryptoProvider) {
    ...
    let default_secret = Secret::random(ciphersuite, None).expect("Not enough randomness.");
    let draft_secret =
        Secret::random(ciphersuite, ProtocolVersion::Mls10Draft11).expect("Not enough randomness.");
