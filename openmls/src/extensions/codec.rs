...
use crate::extensions::{
    ...
    ExternalSendersExtension, GroupContextExtension, RatchetTreeExtension,
...
impl Size for GroupContextExtension {
    #[inline]
    fn tls_serialized_len(&self) -> usize {
        let extension_type_length = 2;
        ...
        // This will be caught when (de)serializing.
        let extension_data_len = match self {
            GroupContextExtension::RequiredCapabilities(e) => e.tls_serialized_len(),
            GroupContextExtension::ExternalSenders(e) => e.tls_serialized_len(),
            GroupContextExtension::Unknown(_, e) => e.0.len(),
        };
...
impl Size for &GroupContextExtension {
    #[inline]
    fn tls_serialized_len(&self) -> usize {
        GroupContextExtension::tls_serialized_len(*self)
    }
}

impl Serialize for GroupContextExtension {
    fn tls_serialize<W: Write>(&self, writer: &mut W) -> Result<usize, tls_codec::Error> {
        // First write the extension type.
        let written = self.extension_type().tls_serialize(writer)?;
        ...
        let extension_data_written = match self {
            GroupContextExtension::RequiredCapabilities(e) => e.tls_serialize(&mut extension_data),
            GroupContextExtension::ExternalSenders(e) => e.tls_serialize(&mut extension_data),
            GroupContextExtension::Unknown(_, e) => extension_data
                .write_all(e.0.as_slice())
                .map(|_| e.0.len())
                .map_err(|_| tls_codec::Error::EndOfStream),
        }?;
...
impl Serialize for &GroupContextExtension {
    fn tls_serialize<W: Write>(&self, writer: &mut W) -> Result<usize, tls_codec::Error> {
        GroupContextExtension::tls_serialize(*self, writer)
    }
}

impl Deserialize for GroupContextExtension {
    fn tls_deserialize<R: Read>(bytes: &mut R) -> Result<Self, tls_codec::Error> {
        // Read the extension type and extension data.
        let extension_type = ExtensionType::tls_deserialize(bytes)?;
        let extension_data = VLBytes::tls_deserialize(bytes)?;
        ...
        match extension_type {
            ExtensionType::RequiredCapabilities => Ok(GroupContextExtension::RequiredCapabilities(
                RequiredCapabilitiesExtension::tls_deserialize(&mut extension_data)?,
            )),
            ExtensionType::ExternalSenders => Ok(GroupContextExtension::ExternalSenders(
                ExternalSendersExtension::tls_deserialize(&mut extension_data)?,
            )),
            ExtensionType::Unknown(unknown) => Ok(GroupContextExtension::Unknown(
                unknown,
                UnknownExtension(extension_data.to_vec()),
            )),
...
impl DeserializeBytes for GroupContextExtension {
    fn tls_deserialize_bytes(bytes: &[u8]) -> Result<(Self, &[u8]), tls_codec::Error>
    where
        Self: Sized,
    {
        let mut bytes_ref = bytes;
        let extension = GroupContextExtension::tls_deserialize(&mut bytes_ref)?;
        let remainder = &bytes[extension.tls_serialized_len()..];
        Ok((extension, remainder))
    }
}
