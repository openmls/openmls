...
impl MlsCiphertext {
    ...
    pub(crate) fn to_plaintext(
        ...
        let mls_ciphertext_content =
            self.decrypt(ciphersuite, backend, ratchet_key, &ratchet_nonce)?;
...
impl MlsSenderData {
    /// Build new [`MlsSenderData`] for a key package reference.
    pub(crate) fn new(sender: &KeyPackageRef, generation: u32, reuse_guard: ReuseGuard) -> Self {
        MlsSenderData {
            sender: sender.clone(),
            generation,
            reuse_guard,
