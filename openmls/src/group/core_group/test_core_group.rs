...
#[apply(ciphersuites_and_backends)]
fn test_failed_groupinfo_decryption(ciphersuite: Ciphersuite, backend: &impl OpenMlsProvider) {
    let epoch = 123;
...
fn test_psks(ciphersuite: Ciphersuite, backend: &impl OpenMlsProvider) {
    ...
    let secret = Secret::random(ciphersuite, backend.rand(), None /* MLS version */)
        .expect("Not enough randomness.");
...
fn test_staged_commit_creation(ciphersuite: Ciphersuite, backend: &impl OpenMlsProvider) {
    ...
    assert_eq!(
        group_bob.export_secret(backend.crypto(), "", b"test", ciphersuite.hash_length()),
        alice_group.export_secret(backend.crypto(), "", b"test", ciphersuite.hash_length())
    );
    assert_eq!(
