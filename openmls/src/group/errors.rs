...
pub enum VcGroupCreationJoinError<StorageError> {
    ...
    EpochIdMismatch,
    /// The creator leaf's derivation info carries no group creation secret, so
    /// it is not a group creator's leaf.
    #[error("The creator leaf carries no group creation secret.")]
    MissingGroupCreationSecret,
