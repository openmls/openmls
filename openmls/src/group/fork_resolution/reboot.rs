...
/// Lets you map any value, instead of just Options, Results and Iterators. This makes it easier to
/// apply the builder refinement closure.
trait AnyMap: Sized {
    fn map<T: Sized, F: FnMut(Self) -> T>(self, mut f: F) -> T {
        f(self)
    }
}
