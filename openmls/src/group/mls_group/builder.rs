...
impl MlsGroupBuilder {
    ...
    ///
    /// The creator's leaf is derived from a `leaf_node` operation secret of
    /// that epoch (so sibling emulator clients can reconstruct its key
    /// material), and the epoch-0 `epoch_secret` is carried, encrypted, in the
    /// leaf's `DerivationInfoTBE` `group_creation_secret`. Siblings bootstrap
    /// into the group with [`MlsGroup::vc_join_at_creation`].
    ///
...
/// leaf).
///
/// The creator's leaf is derived from a `leaf_node` operation secret of the
/// emulation epoch `epoch_id`, and the sampled epoch-0 `epoch_secret` is
/// carried, AEAD-encrypted, in the leaf's `DerivationInfoTBE`
/// `group_creation_secret`. A sibling emulator client reconstructs this exact
/// state with [`MlsGroup::vc_join_at_creation`]. Because the `epoch_secret` is
/// transmitted rather than re-derived, this path bypasses the joiner key
/// schedule entirely.
///
/// [`MlsGroup::vc_join_at_creation`]: crate::group::MlsGroup::vc_join_at_creation
#[cfg(feature = "virtual-clients-draft")]
...
fn build_vc_internal<Provider: OpenMlsProvider>(
    ...
    use openmls_traits::{random::OpenMlsRand as _, storage::StorageProvider as _};

    use crate::{
        components::vc_derivation_info::{
            load_vc_epoch_state_and_tree, DerivationInfo, DerivationInfoTbe, GroupCreationSecret,
    ...
    // Load the emulation epoch state and operation tree, allocate a `leaf_node`
    // generation (operation context = the new group's id), and persist the
    // advanced tree right away.
    ...
    // Derive the creator leaf's encryption keypair (group ciphersuite) and
    // sample the epoch-0 epoch secret carried to siblings in the leaf.
    ...
    let epoch_secret = provider
        .rand()
        .random_vec(ciphersuite.hash_length())
        .map_err(|_| LibraryError::custom("Not enough randomness"))?;

    // Wrap the derivation info (carrying the epoch secret) under the per-epoch
    // AEAD key, bound to the leaf via its serialized encryption key.
    ...
    let tbe = DerivationInfoTbe::LeafNode {
        ...
        group_creation_secret: Some(GroupCreationSecret::from_slice(&epoch_secret)),
    };
    ...
    // Build the single-leaf tree with the derived leaf and initial context.
    ...
    // Derive epoch-0 secrets directly from the sampled epoch secret.
    let epoch_secrets =
    ...
    // operations in this group resolve the right emulation state.
