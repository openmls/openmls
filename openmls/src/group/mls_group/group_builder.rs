...
impl<'a> GroupBuilder<Init<'a>> {
    ...
    pub fn read_key_package<Provider: OpenMlsProvider>(
        ...
        // Sub-group branching always joins with a locally stored key package, so
        // the resumption PSK store returned here is not used: `read_psks` builds
        // its own store and injects the branch PSK.
        let (_resumption_psk_store, key_material) =
            keys_for_welcome(self.stage.config, &self.stage.welcome, provider)?;
        let key_package_bundle = match key_material {
            WelcomeKeyMaterial::KeyPackage(bundle) => *bundle,
            #[cfg(feature = "virtual-clients-draft")]
            WelcomeKeyMaterial::VirtualClient(_) => {
...
impl<'a> GroupBuilder<Psks<'a>> {
    ...
    pub fn key_schedule<Provider: OpenMlsProvider>(
        ...
        let (epoch_secrets, verifiable_group_info) = prepare_key_schedule(
            provider.crypto(),
        ...
        Ok(ProcessedWelcome {
            ...
            key_material: WelcomeKeyMaterial::KeyPackage(Box::new(
                self.stage.decrypted.key_package.key_package_bundle,
            )),
        })
    }
