...
use crate::{
    binary_tree::array_representation::LeafNodeIndex, group::ReAddMembersError,
...
impl MlsGroup {
    ...
    ///
    /// For the best performance it is important to sort the remove indices and
    /// key packages in the same order. If `sort` is set to `true`, we sort
    /// the indices internally, based on the credential. Note that this only
    /// works if the credential doesn't change and the credential allows uniquely
    ...
    pub fn readd_members<Provider: OpenMlsProvider>(
        ...
        (MlsMessageOut, MlsMessageOut, Option<GroupInfo>),
        ReAddMembersError<Provider::StorageError>,
    > {
        self.is_operational()?;
        ...
        let welcome = welcome
            ...
            .ok_or(ReAddMembersError::LibraryError(LibraryError::custom(
                "A welcome must have been produced",
            )))?;
