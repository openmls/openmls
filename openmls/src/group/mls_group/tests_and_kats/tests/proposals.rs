...
// #[openmls_test::openmls_test]
// fn invalid_group_extension_proposals_are_filtered(
//     ciphersuite: Ciphersuite,
//     provider: &impl crate::storage::OpenMlsProvider,
// ) {
//     // Framing parameters
...
//
//     // Define identities
//     let (_alice_credential, _alice_key_package_bundle, alice_signer, _alice_pk) =
//         setup_client("Alice", ciphersuite, provider);
//
//     let ratchet_tree = {
...
//
//         RatchetTree::try_from_nodes(
//             ciphersuite,
//             provider.crypto(),
//             vec![Some(node_in.clone())],
//             &GroupId::random(provider.rand()),
//         )
//         .unwrap()
//     };
//
//     let ratchet_tree_ext = RatchetTreeExtension::new(ratchet_tree);
//
//     let invalid_proposals = vec![
//         Proposal::GroupContextExtensions(GroupContextExtensionProposal::new(Extensions::single(
//             Extension::ApplicationId(ApplicationIdExtension::new(&[0])),
//         ))),
//         Proposal::GroupContextExtensions(GroupContextExtensionProposal::new(Extensions::single(
//             Extension::RatchetTree(ratchet_tree_ext),
//         ))),
//         Proposal::GroupContextExtensions(GroupContextExtensionProposal::new(Extensions::single(
//             Extension::ExternalPub(ExternalPubExtension::new(HpkePublicKey::new(vec![0]))),
//         ))),
//     ];
//
//     let group_context = GroupContext::new(
...
//         let mls_plaintext_appl_id = AuthenticatedContent::member_proposal(
//             framing_parameters,
//             LeafNodeIndex::new(0),
//             proposal,
//             &group_context,
...
//                 ciphersuite,
//                 provider.crypto(),
//                 mls_plaintext_appl_id,
//             )
//             .unwrap(),
//         );
//
//         let (proposal_queue, _own_update) = ProposalQueue::filter_proposals(
//             proposal_store.proposals().map(Clone::clone),
//             LeafNodeIndex::new(0),
//         )
//         .expect("Could not create ProposalQueue.");
//
//         assert!(proposal_queue.is_empty());
//     }
// }
