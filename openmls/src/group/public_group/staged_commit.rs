...
use crate::{
    ...
    prelude_test::Sender,
    schedule::{
        message_secrets::MessageSecrets, psk::PskSecret, GroupEpochSecrets, InitSecret,
...
impl PublicGroup {
    ...
    pub(crate) fn stage_commit_public<'a>(
        &self,
        ...
        let (commit, proposal_queue, sender_index) =
            self.validate_commit(mls_content, proposal_store, own_leaf_nodes, backend)?;
    ...
    pub(crate) fn stage_commit_private<'a>(
        &self,
        mls_content: &AuthenticatedContent,
        proposal_store: &ProposalStore,
        own_leaf_nodes: &[OpenMlsLeafNode],
        backend: &impl OpenMlsCryptoProvider,
        private_group_params: PrivateGroupParams<'a>,
        ...
        // XXX: Private key material required here
    ...
    /// group.
    pub(crate) fn stage_commit<'a>(
        &self,
        mls_content: &AuthenticatedContent,
        proposal_store: &ProposalStore,
        own_leaf_nodes: &[OpenMlsLeafNode],
        backend: &impl OpenMlsCryptoProvider,
        private_group_params: impl Into<Option<PrivateGroupParams<'a>>>,
    ) -> Result<StagedCommit, StageCommitError> {
        let private_group_params = private_group_params.into();
        let ciphersuite = self.ciphersuite();
        ...
        let processor_leaf_index: Option<LeafNodeIndex> =
            private_group_params.map(|params| params.own_leaf_index);

        // XXX: Private key material required here
        let apply_proposals_values = diff.apply_proposals(&proposal_queue, processor_leaf_index)?;
        ...
        let (commit_secret, new_keypairs, new_leaf_keypair_option) =
            if let Some(path) = commit.path.clone() {
                ...
                }) = private_group_params
                {
                    ...
                    // No private values were given, so there is no private key material to return.
                    (
                        CommitSecret::zero_secret(ciphersuite, self.version()),
                        vec![],
                        None,
