...
//!
//! Every leaf the library builds goes through `LeafNodeTbs::new`, which calls
//! `Capabilities::reconcile()` (see
//! `openmls/src/treesync/node/leaf_node/capabilities.rs`) to check the leaf's
//! capabilities against its own ciphersuite, credential type, extensions, and
//! the group's `RequiredCapabilitiesExtension`. By default
//! (`CapabilitiesPolicy::Reject`) a mismatch fails construction outright;
//! `CapabilitiesPolicy::Widen` instead adds whatever's missing. This
//! supersedes the narrower, ad hoc `ensure_ciphersuite()` fix this file used
//! to document as a partial one.
...
/// credential type, even when capabilities are left at their default.
///
/// `Capabilities::default()`'s credential list is hardcoded to
/// `[CredentialType::Basic]` (see
/// `openmls/src/treesync/node/leaf_node/capabilities.rs`), independent of
/// the credential actually passed to `MlsGroup::builder()`. `reconcile()`
/// catches the mismatch at construction time and rejects it.
#[openmls_test::openmls_test]
...
///
/// `reconcile()` threads the group's `RequiredCapabilitiesExtension` down to
/// the creator's own leaf construction, so a mismatch is now rejected there
/// rather than slipping through uncaught.
#[openmls_test::openmls_test]
...
///
/// This is the `leaf_extensions` arm of `reconcile()`, distinct from the
/// `RequiredCapabilitiesExtension` arm below it: here the extension is one the
...
/// `CapabilitiesPolicy::Widen` covers *every* dimension `reconcile()` checks,
/// not just the ciphersuite.
///
/// The capabilities here start empty and the group demands something in each
/// dimension at once, so this fails if any one arm of `reconcile()` stops
/// widening.
#[openmls_test::openmls_test]
fn widening_covers_every_capability_dimension() {
    let alice_credential = generate_credential_with_key(
        "Alice".into(),
        ciphersuite.signature_algorithm(),
        &Provider::default(),
    ...
    assert!(
        capabilities.extensions().contains(&required_extension),
        "widening did not add the group's required extension type"
    );
    assert!(
        capabilities.proposals().contains(&required_proposal),
        "widening did not add the group's required proposal type"
    );
    assert!(
        capabilities.credentials().contains(&required_credential),
        "widening did not add the group's required credential type"
    );
}

/// The protocol version is the one dimension `reconcile()` adds even under
