...
#[openmls_test]
async fn external_join_add_proposal_should_succeed() {
    ...
    for policy in WIRE_FORMAT_POLICIES {
        let ProposalValidationTestSetup {
            alice_group,
            bob_group,
        } = validation_test_setup(policy, ciphersuite, alice_provider, bob_provider).await;
...
#[openmls_test]
async fn external_join_add_proposal_should_be_signed_by_key_package_it_references() {
    let alice_provider = &Provider::default();
    let bob_provider = &Provider::default();
    let charlie_provider = &Provider::default();
