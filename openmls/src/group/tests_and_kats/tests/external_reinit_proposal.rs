//! External-sender ReInit proposal.
//!
//! RFC 9420 §12.1.8.2 permits an external sender (configured via the
//! `ExternalSendersExtension`) to send `ReInit` proposals. This test checks that
//! OpenMLS both *creates* such a proposal ([`ExternalProposal::new_reinit`]) and
//! *accepts* it on the receiving side (rather than rejecting it with
//! `UnsupportedProposalType`), and that committing it suspends the old group as
//! usual.
...
// An external sender's ReInit proposal is accepted on processing (it used to be
// rejected as `UnsupportedProposalType`), and committing it suspends the old
// group.
#[openmls_test]
fn external_reinit_proposal_should_be_accepted() {
    ...
    let (mut alice_group, alice_credential) = setup(
        ciphersuite,
        alice_provider,
        bob_provider,
    ...
    // The DS crafts an external ReInit proposal describing the successor group.
    let reinit_proposal = ReInitProposal::new(
        GroupId::from_slice(b"Successor Group"),
        ProtocolVersion::Mls10,
        ciphersuite,
        Extensions::empty(),
    );
    let external_reinit_proposal: MlsMessageIn = ExternalProposal::new_reinit(
        reinit_proposal,
    ...
    let processed_message = alice_group
        .process_message(
            alice_provider,
            external_reinit_proposal
                .clone()
                .try_into_protocol_message()
                .unwrap(),
        )
        .expect("external ReInit proposal must be accepted");
    ...
    assert!(
        ...
        "expected a ReInit proposal"
    );
    assert!(matches!(proposal.sender(), Sender::External(_)));

    // Alice stores the queued proposal and commits it. The commit carries only
    // the ReInit; merging it suspends the old group.
    alice_group
    ...
    assert!(
        ...
        "committing the external ReInit must suspend the old group"
    );
}

// A ReInit proposal from an *unauthorized* external sender is still rejected.
#[openmls_test]
fn external_reinit_proposal_should_fail_when_invalid_external_senders_index() {
    let alice_provider = &Provider::default();
    let bob_provider = &Provider::default();
    let ds_provider = &Provider::default();
