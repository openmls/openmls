...
fn test_max_forward_distance(ciphersuite: Ciphersuite, backend: &impl OpenMlsProvider) {
    ...
    let secret = Secret::random(ciphersuite, backend.rand(), ProtocolVersion::Mls10)
    ...
    ratchet1
        .secret_for_decryption(ciphersuite, backend.crypto(), u32::MAX - 1, configuration)
...
fn test_out_of_order_generations(ciphersuite: Ciphersuite, backend: &impl OpenMlsProvider) {
    ...
    for i in 0..configuration.out_of_order_tolerance() * 2 {
        let _secret = ratchet1
            .secret_for_decryption(ciphersuite, backend.crypto(), i, configuration)
            .expect("Expected decryption secret.");
    }
...
fn test_forward_secrecy(ciphersuite: Ciphersuite, backend: &impl OpenMlsProvider) {
    ...
    let _ratchet_secrets = ratchet
        .secret_for_decryption(ciphersuite, backend.crypto(), 0, configuration)
    ...
    let _ratchet_secrets = ratchet
        .secret_for_decryption(ciphersuite, backend.crypto(), 10, configuration)
        .expect("Error ratcheting forward.");

    // First, let's make sure that the window works.
    let err = ratchet
        .secret_for_decryption(ciphersuite, backend.crypto(), 5, configuration)
        .expect_err("No error when trying to retrieve key outside of tolerance window.");
    assert_eq!(err, SecretTreeError::TooDistantInThePast);
    ...
    for generation in 10 - configuration.out_of_order_tolerance() + 1..10 {
        ...
        let err = ratchet
            .secret_for_decryption(ciphersuite, backend.crypto(), generation, configuration)
...
fn sender_ratchet_generation_overflow() {
    ...
    let _ = ratchet
        .ratchet_forward(backend.crypto(), ciphersuite)
        .expect("error ratcheting forward");
    let err = ratchet
        .ratchet_forward(backend.crypto(), ciphersuite)
