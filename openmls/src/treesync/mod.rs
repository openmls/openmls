...
impl TreeSync {
    ...
    /// Get an iterator over the leaves as `(KeyPackageRef, &KeyPackage)` tuples
    /// in the tree.
    pub(crate) fn key_packages(
        &self,
    ) -> Result<impl Iterator<Item = Option<(KeyPackageRef, &KeyPackage)>>, TreeSyncError> {
        let tsn_nodes = self.tree.iter().filter(|tsn| tsn.node().is_some());
        let leaves = tsn_nodes.map(|tsn_node| {
            if let Some(ref node) = tsn_node.node() {
                if let Ok(leaf) = node.as_leaf_node() {
                    if let Some(kpr) = leaf.key_package_ref().cloned() {
                        Some((kpr, leaf.key_package()))
                    } else {
                        None
                    }
                } else {
