...
fn test_free_leaf_computation(ciphersuite: Ciphersuite, backend: &impl OpenMlsCryptoProvider) {
    ...
    let kpb_0 = KeyPackageBundle::new(&[ciphersuite], &cb_0, backend, Extensions::empty())
    ...
    .expect("error creating credential_bundle");
    let kpb_3 = KeyPackageBundle::new(&[ciphersuite], &cb_3, backend, Extensions::empty())
        .expect("error creating kpb");
    ...
    .expect("error creating credential_bundle");
    let kpb_2 = KeyPackageBundle::new(&[ciphersuite], &cb_2, backend, Extensions::empty())
        .expect("error creating kpb");
