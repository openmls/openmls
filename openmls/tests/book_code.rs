...
///  - Alice commits a ReInit proposal
...
fn reinit_group() {
    ...
    let bundle = alice_group
        .commit_builder()
        .add_proposal(Proposal::ReInit(Box::new(reinit_proposal.clone())))
        .load_psks(alice_provider.storage())
        .unwrap()
        .build(
            alice_provider.rand(),
    ...
    // Alice exports the ReInitInfo. It holds everything needed for the reinit, alice_group can be discarded.
    let alice_reinit_info = alice_group.reinit_info(reinit_proposal.clone()).unwrap();
    drop(alice_group);
    // ANCHOR_END: reinit_commit_value
    ...
    // Bob extracts the reinit proposal
    let bob_reinit_proposal = staged
        .reinit_proposal()
        .expect("staged commit contains no reinit proposal")
        .clone();
    ...
    // Bob extracts ReInitInfo. It holds everything needed for the reinit, bob_group can be discarded.
    let bob_reinit_info = bob_group.reinit_info(bob_reinit_proposal).unwrap();
    drop(bob_group);
    ...
    // This also sets protocol version, ciphersuite, group_id, and group_context.extensions
    // to the values of the ReInit proposal.
...
fn reinit_group_by_reference_pending_welcome() {
    ...
    // Bob extracts ReInitInfo. It hold everything needed for the reinit, bob_group can be discarded.
    let bob_reinit_info = bob_group.reinit_info(bob_reinit_proposal).unwrap();
    drop(bob_group);
    ...
    let bob_successor = pending_welcome
        .build_from_reinit(bob_provider, bob_reinit_info)
        .unwrap()
        .with_ratchet_tree(alice_successor.export_ratchet_tree().into())
        .build()
        .unwrap()
