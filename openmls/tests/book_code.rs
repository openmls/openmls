...
fn book_operations(ciphersuite: Ciphersuite, backend: &impl OpenMlsCryptoProvider) {
    ...
    let alice_key_package = generate_key_package(&[ciphersuite], &alice_credential, backend);
    ...
    let mut alice_group = MlsGroup::new(backend, &mls_group_config, alice_key_package)
        .expect("An unexpected error occurred.");
