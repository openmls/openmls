...
fn mls_group_operations(ciphersuite: Ciphersuite, backend: &impl OpenMlsCryptoProvider) {
    ...
    for wire_format_policy in WIRE_FORMAT_POLICIES.iter() {
        ...
        let queued_message = alice_group
            .create_message(crypto, &alice_signer, message_alice)
        ...
        let queued_message = charlie_group
            .create_message(crypto, &charlie_signer, message_charlie)
            .expect("Error creating application message");
        ...
        assert!(bob_group
            .create_message(crypto, &bob_signer, b"Should not go through")
            .is_err());
        ...
        let (queued_message, _) = alice_group
            .propose_remove_member(crypto, &alice_signer, charlie_group.own_leaf_index())
