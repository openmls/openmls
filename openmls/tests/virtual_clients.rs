...
fn bound_group_fails_closed_when_derivation_state_missing_on_send() {
    ...
    // Drop the binding, delete the state, then put the binding back. That
    // leaves the group bound to an epoch whose state is gone, which is the
    // situation a corrupted or partially restored store can produce.
    provider
        .storage()
