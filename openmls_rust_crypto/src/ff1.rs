...
mod tests {
    ...
    /// decrypt mismatch between two openmls clients. The same vector is
    /// pinned in `openmls_libcrux_crypto` so the providers stay in
    /// agreement.
    #[test]
    fn fixed_vector() {
        let key = [
        ...
        let permuted = encrypt(&key, x).expect("encrypt");
        // Round-trip is the load-bearing check. The absolute output is
        // pinned below.
