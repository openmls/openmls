...
fn assert_send<F: std::future::Future + Send>(future: F) -> F {
    future
}

#[tokio::test(flavor = "multi_thread")]
async fn overlapping_calls_on_one_provider() {
    let group_id = TestGroupId(b"overlapping-group".to_vec());
    let tree = TestBlob(b"tree".to_vec());
    let group_context = TestBlob(b"context".to_vec());
    ...
    let (tree_written, context_written) = tokio::join!(
        assert_send(storage.write_tree(&group_id, &tree)),
        assert_send(storage.write_context(&group_id, &group_context)),
    );
    tree_written.unwrap();
    context_written.unwrap();

    let (tree_read, context_read) = tokio::join!(
        storage.tree::<_, TestBlob>(&group_id),
        storage.group_context::<_, TestBlob>(&group_id),
    );
    assert_eq!(Some(tree), tree_read.unwrap());
    assert_eq!(Some(group_context), context_read.unwrap());
}
